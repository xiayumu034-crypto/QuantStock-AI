...
def main():
    ...
    print(f"[Infer v19 Ensemble] Qlib 初始化成功.")
    ...
    df = df.dropna()
    if df.empty:
        print("[Infer] 获取的最新数据为空")
        return

    latest_date = df.index.get_level_values('datetime').max()
    print(f"[Infer v19 Ensemble] 最新可用数据日期: {latest_date}")
    ...
    print(f"[Infer v19 Ensemble] 预测完成，共生成 {len(results)} 只股票的预测结果，保存至 {out_file}")
