...
// ========== 全局状态 ==========
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setupEvents();
        refreshAll();
        loadPreviousScreenerStatus(false); // 默认尝试加载技术的缓存
        refreshWeeklyPool();
        pollTrainStatus();
        setInterval(refreshAll, refreshInterval * 1000);
        
        // 初始加载信号质量
        loadSignalQuality(document.getElementById('modelVersionSelect').value || 'v20');
    });
    ...
    function executeV20ToTrade() {
        const btn = document.getElementById('btnExecuteV20');
        const oldText = btn.innerText;
        btn.disabled = true;
        btn.innerText = '正在执行下单...';
        
        fetch('/api/sim/execute_v20', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ mode: 'sim', threshold: 0.50 })
        })
        .then(r => r.json())
        .then(d => {
            btn.disabled = false;
    ...
    updateRankings();
    setInterval(updateRankings, 5 * 60 * 1000); // 5分钟更新一次，避免请求太频繁被封
    ...
    // V0 Screener Logic
    let v0PollInterval = null;
    let currentV0UseAI = false;
