...
def main():
    ...
    # Reconstruct the feature dict exactly as in train_qlib_v18.py
    features = {
    ...
    # We evaluate on the test period defined in train_qlib_v18.py
    ...
    # apply cross sectional norm
    df_norm = df.groupby(level='datetime', group_keys=False).apply(cross_sectional_norm)
    df[all_names] = df_norm[all_names]

    preds = model.predict(df[all_names])
    df['Predict'] = preds

    total_samples = len(df)
    
    # 1. Directional accuracy (Predict > 0 & Label > 0) or (Predict < 0 & Label < 0)
    correct = (np.sign(df['Predict']) == np.sign(df['Label']))
    overall_win = correct.mean()
    ...
    print(f"--- 近期测试集评估 (2026-03-01 至今) ---")
    print(f"总评估样本: {total_samples}")
    print(f"整体多空方向胜率: {overall_win * 100:.2f}%")
    print(f"=====================================")
