...
请包含以下模块：
1. **📌 事件定性**：一句话总结该事件的本质。如果该新闻属于公司常规公告（如高管减持、资金置换等），且不具备宏观发酵潜力，请直接定性为“常规公司公告”。
2. **🕸️ 逻辑推演图谱数据**：必须提供一个 JSON 格式的树状数据，包裹在 ```json 和 ``` 之间，用于前端 ECharts 渲染。
如果事件是常规个股公告，只需构建简化的“事件 -> 公司基本面 -> 情绪影响”即可，不要强行编造宏观影响。
JSON格式必须严格如下（name字段代表节点名称，children代表子节点列表）：
```json
{
  "name": "核心事件",
  "children": [
...
```
3. **🎯 关联板块与个股画像**：指出最直接受益的 A 股板块，并描述该板块中哪类股票最容易成为资金龙头。
"""
...
- **个股画像**：建议重点关注具有实际产能释放预期、市值在 100-300亿之间、机构资金介入较深的行业中军（如中国海油等逻辑标的）。"""
        ...
        return {"html": f"<div class='alert alert-danger'>AI 推理引擎罢工了: {str(e)}</div>", "graph_data": None}